...
The inspector (<kbd>⌘4</kbd>, or the toolbar button) is where the selected entity's **facts** live — everything about a chapter or character that isn't its prose. It's the same panel behind the editor, the graph's glance panel, and the timeline. It starts closed; open it when you need it, and it follows whichever pane is focused when the editor is [split](/guide/editor#splitting).
